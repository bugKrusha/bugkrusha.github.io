...
## Introduction
Can I have a `T`? Perplexed? Understandably so. In the real world, it is virtually impossible for you to respond to this without more context or specialization. Do I need a medium, do I need it hot, or should it be uppercase? However, in Software engineering and Swift specifically, when you see the latter, we are employing one of the most powerful paradigms in modern software engineering, _Generics_.
...
*****
```
Early on in our careers, we learn to parameterize functions as we become frustrated with the futility of hard coding values into our programs. If we wanted to draw a triangle with ascii art, as shown here, we could start like this :weary:
...
```
#### Type
A stack is a common data structure that is described as a FILO data structure. That is, the first element will be the last out. They are backed by an array and have two important functions. `push` and `pop`, where `push` adds and element and `pop` removes the last element that was added. If we wanted a stack of emojis, we could have something like this.
...
```
Then, we realize we need a stack for numbers. We can make something like this. 
...
Not only is it tedious to repeat similar definitions like this, it is unsafe and difficult to maintain. The definition of the backing  `data` array is essentially identical, as is the `push` and `pop` functions. The only difference is the type in the backing array and the name of the `Stack`. Abstracting away this hard coded type, leads to the creation of a single unifying polymorphic type, parameterized by another type, the type that will be the backing `data` array. This is called _parametric polymorphism_. At work here is the fact that the instantiated behavior is independent of the type passed in. We just want to be able to add, remove and store elements, regardless of the element.
...
#### Function
One of my favorite form of *Genericity* is genericity by function, often called higher-order functions. This possible in Swift because functions are first class citizens. Suppose you had a list of strings that you wanted to convert to lowercase for standardization. You may write something like this:
...
```
Functions that follow this pattern are very common. The only difference here is the function to be applied to each element in the list. For the first case, the `lowercased` function and for the second, it is this condition, that the number is even. What is common here is traversing the list and applying that func to the list.This is called `map` and can be defined as follows.
...
This isn’t just genericity by value where the value is just a function. The implications here are far reaching, proving incredibly powerful customization points for engineers without extending the language. Here is how this looks with `map` with the former examples.
...
Let’s take this even further. Say we have two lists that we want to append to together. We could write that in a polymorphic way like this.
...
```
And say we have a list of numbers for which we would like to obtain a sum. We could write it like this.
...
These programs can be unified since they both traverse the list in the same way. Stepping back, we can see that they have a common pattern of recursion called a fold. That is, given a list, recursively apply a combine operation, build up a return value by recombining the results of recursively processing the elements in that list. In Swift, this is called `reduce` and can be defined as follows.
...
let bigListEnergy = ["ONE", "LOVE", "mi", "bredda"].reduce("", +)
```
## JSON
You might be familiar with JSON,  Javascript object notation is a text format that is used to serialize structured data, with the primary purpose of transmitting data between a server and an application. Here is an example:

Modeling this is reasonably straightforward and might look like this:
...
Things can get complicated very fast when modeling JSON however and there are OPINIONS about how it should be formatted. So, there is now a JSON API that helps to standardize things. Broadly, JSON that conforms to these specifications might look like this:
...
```
I will call the object that represents `data ResourcePacket`. This conforms to `ResourceRequestable`. The associated type is purely Generic with no inclusion constraints. That is, it can be anything. 
...
### Conclusion
When building software systems with generics, we can decompose components in a manner that makes little to no assumptions about the other components, providing for maximum flexibility in composition. Every data type has a set of axioms. Generics emerged out of the insight that highly reusable software components must be built with a minimum set of such requirements. However, we can’t fully leverage generic programming by just identifying a set of minimal axioms/requirements for a single data type or algorithm. We must identify a set of axioms for a collection of similar types. This way we can build remarkably powerful systems with reusable components that enhance the safety and reliability of said systems. Combined with protocols, there is nothing you can’t build. 
